...
class ResultFormat(object):
    ...
    def write_result(self, img_name, outputs):
        ...
        elif 'Sample' in self.data_type:
            self._write_result_Sample(img_name, outputs)
    ...
    #Sample data 추가
    def _write_result_Sample(self, img_name, outputs):
        assert self.result_path.endswith('.zip'), 'Error: Sample result should be a zip file!'
    ...
    def _write_result_tt(self, image_name, outputs):
        ...
        import numpy as np
        for i, bbox in enumerate(bboxes):
            bbox = np.array(bbox).reshape(-1, 2)[:, ::-1].reshape(-1)
            values = [int(v) for v in bbox]
            line = "%d" % values[0]
    ...
    def _write_result_ctw(self, image_name, outputs):
        ...
        for i, bbox in enumerate(bboxes):
            bbox = np.array(bbox).reshape(-1, 2)[:, ::-1] # [:, ::-1]
            bbox = unique(bbox, axis=0) # remove duplicate points
            bbox = bbox.reshape(-1)
            values = [int(v) for v in bbox]
            line = "%d" % values[0]
            for v_id in range(1, len(values)):
