...
try:
    ...
    if ccl_cuda is None:
        print("ccl_cuda is not available!")
        ccl_cuda = None
except:
...
class FASTHead(nn.Module):
    ...
    def get_results(self, out, img_meta, cfg, scale=2):
        ...
        if kernels.is_cuda and ccl_cuda is not None:
            labels_ = ccl_cuda.ccl_batch(kernels)  # B*160*160
        else:
            # Use CPU fallback with cv2.connectedComponents
            kernels_cpu = kernels.cpu().numpy()
            labels_ = []
            for kernel in kernels_cpu:
